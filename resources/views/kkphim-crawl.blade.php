...
@section('header')
    <section class="container-fluid">
        <h2>
            <span class="text-capitalize">KKPhim</span>
            <small>Crawler</small>
        </h2>
    </section>
@endsection
    ...
    <script>
        ...
        $('.btn-load').click(function(e) {
            ...
            const fetchApi = async (link, from, to) => {
                isFetching = true;
                const response = await fetch("{{ backpack_url('plugin/vsmov-crawler/fetch-kkphim') }}?" +
                    new URLSearchParams({
                        link,
                        from,
                        to
        ...
        const processMovie = async (slug) => {
            ...
            const response = await fetch("{{ backpack_url('plugin/vsmov-crawler/crawl-kkphim') }}", {
                method: 'POST',
                headers: {
        ...
        $("#movies-get-handle").click(async function() {
            const apiDomain = "{{ config('vsmov_crawler.domain_kkphim', 'https://phimapi.com') }}";
